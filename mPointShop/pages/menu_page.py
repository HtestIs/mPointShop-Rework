...
# URL (this one is required, admin can change the default menu, but the URL should remain the same for each page)
    ...
    VOUCHER_SCAN_URL = "/merchant-scan/voucher"
# LOCATORS
    ...
    @allure.step("Navigate to Voucher Scan")
    def navigate_to_voucher_scan(self):
        self.open(self.VOUCHER_SCAN_URL)
        return VoucherScan(self.driver)
    @allure.step("Log out")
    def log_out(self):
