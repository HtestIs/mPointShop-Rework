...
- Python 3.10+ (use the repository `requirements.txt`)
- Git (to clone)
...
Copy `.env.example` to `.env` and populate values required for your environment. Common keys used by tests and fixtures:
...
The test runner uses `--env` to choose an environment entry from `config/env_config.py`. For CI runs set `CI=true` in the environment to run browsers in headless mode automatically.
